...
describe("User Service", () => {
  ...
  it("should trigger a bad request error if user is too young", async () => {
    try {
      await createUser({
        name: "Louis A",
        birthday: new Date(new Date().getFullYear() - 10, 8, 13),
      });
      assert.fail("MIN_USER_AGE should trigger an error.");
    } catch (e) {
